...
var server = http.createServer(function(request, response){
    ...
    /* 아래의 코드들이 동작하기 전에, 서버는 클라이언트가 원하는게 무엇인지 구분을 먼저 해야한다.
        == 요청 분석 
        요청 분석을 들어가려면, 요청 정보를 담고있는 request 객체를 이용해야 한다.!! */
    console.log("클라이언트가 요청시 사용한 url", request.url);


    /* 1) 클라이언트가 회원가입 양식을 보기 원하면, 아래의 코드를 수행. */
    ...
    /* 2) 가입을 원하는 경우엔 db에 넣어주고, */

    
    
    /* 3) 이미지를 원하는 경우엔 이미지를 보여주고, */



    /* 4) 오디오 파일을 원하는 경우엔 오디오 파일을 들려준다. */
