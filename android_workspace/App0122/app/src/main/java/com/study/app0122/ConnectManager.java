...
public class ConnectManager extends Thread {
    ...
    public int requestByGet() {    // Get방식으로 요청을 시도하는 메서드
        ...
        try {
            ...
            code = con.getResponseCode();    // 서버로부터 받은 응답코드 반환 (이 시점에 이미 서버에 요청을 완료하고 응답도 받은 상태)
            Log.d(TAG, "서버로부터 받은 응답 코드는 " + code);

            // 이 시점이 바로 통신이 완료된 시점이므로, 여기서 무언가를 해야한다

            Log.d(TAG, "서버로부터 받은 데이터는 " + sb.toString());
            //mainActibity.printData(sb.toString());    // 사용자 정의 thread는 디자인 접근 불가
            ...
            //mainActivity.handler.sendMessage();    // handler의 handlerMesasge()를 호출하게 됨
