...
public class GalleryController {
	...
	//Spring Framework은 upload componenet로 apache fileupload를 사용함
	@RequestMapping(value="/regist", method=RequestMethod.POST)
	public String regist(Gallery gallery, HttpServletRequest request) {
		System.out.println("등록을 원하시나요?");
		...
		gallery.setFilename(filename);  //VO에 새로운 파일명 저장
		ServletContext context = request.getServletContext();
		String realPath = context.getRealPath("/data");

		//이클립스 내부 톰캣인 경우, 실제 경로와는 다른 경로에 저장된다
		//개발시에 그 경로를 확인하려면 출력해볼 필요가 있다
