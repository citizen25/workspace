...
public class ProductController implements ServletContextAware {
	 ...
	 * 
	 * @ResponseBody //jsp와 같은 view 페이지가 아닌, 단순 데이터만 전송시 ResponseBody 사용 public
	 * String getSubList(int topcategory_id) { logger.debug("topcategory_id : " +
	 * topcategory_id);
	 * 
	 * List<SubCategory> subList = subCategoryService.selectAllById(topcategory_id);
	 * 
	 * //list를 json으로 형변환해야함 StringBuilder sb = new StringBuilder(); sb.append("{");
	 ...
	 * sb.append("\"name\":\"" + subCategory.getName() + "\""); if(i <
	 * subList.size()-1) { sb.append("},"); } else { sb.append("}"); } }
	 * sb.append("]"); sb.append("}");
	 * 
	 * return sb.toString(); }
	 */
	...
	public String registProduct(Product product, String[] test) {
		 ...
		 * logger.debug("업로드 추가 이미지명 : " +
		 * product.getAddImg()[i].getOriginalFilename()); }
		 */
	...
	public ModelAndView getShopProductList(HttpServletRequest request, int subcategory_id) { // 하위 카테고리의 id
