...
<script>
	...
	//요청이 완료되는 시점에 프로그래스바를 감춘다 --> 비동기 방식 사용
	function regist(){
		//로딩바 시작
		$("#loader").addClass("loader");  //class 동적 적용

		//form 테크의 파라미터들을 전송할 수 있는 상태로 둬야 data키값에 form 자체를 넣을 수 있다
		var formData = $("#member_form").serialize();  //전부 문자열화 시킨다

		$.ajax({
			url: "/shop/member/regist",
			type: "post",
			data: formData,  //form자체를 data 키값에 넣기
	...
	/*
	//요청이 완료되는 시점에 프로그래스바를 감춘다 --> 따라서 동기방식은 사용하지 않는다
	function regist() {
		$("#member_form").attr({
